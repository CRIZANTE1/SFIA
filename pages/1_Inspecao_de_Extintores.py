...
def decode_qr_from_image(image_file):
    """
    Decodifica o QR code. A lógica assume que o QR Code contém o ID do Equipamento.
    """
    try:
        ...
        # Se o seu QR code for composto (ex: 2#1430#EXT#03#...), ajuste o índice abaixo.
        # Caso contrário, ele retornará o texto completo.
        if '#' in decoded_text:
            parts = decoded_text.split('#')
            # Exemplo: pegando o 4º item se o ID estiver nessa posição
...
def find_last_record(df, search_value, column_name):
    ...
    if 'data_servico' in records.columns:
        ...
        # Reatribuição para evitar o warning do inplace=True
                ...
                with st.spinner("Iniciando processo de salvamento..."):
                    ...
                    if st.session_state.service_level in ["Manutenção Nível 2", "Manutenção Nível 3"]:
                        ...
                        try:
                            ...
                            st.error(f"Falha ao fazer upload do PDF: {e}"); st.stop()
                    ...
                    st.balloons()
                    st.session_state.batch_step = 'start'; st.session_state.processed_data = None; st.session_state.uploaded_pdf_file = None
                    st.rerun()
    ...
    with tab_qr:
        ...
        if st.session_state.qr_step == 'inspect':
            if st.session_state.last_record:
                ...
                plano_de_acao_gerado = generate_action_plan(temp_record_for_plan)
                st.info(f"**Plano de Ação Sugerido:** {plano_de_acao_gerado}")

                col_btn1, col_btn2 = st.columns(2)
                with col_btn1:
                    if st.button("✅ Registrar Inspeção", type="primary", use_container_width=True):
                        with st.spinner("Salvando..."):
                            ...
                            new_record.update({
                                ...
                                'aprovado_inspecao': temp_record_for_plan['aprovado_inspecao'],
                                'observacoes_gerais': temp_record_for_plan['observacoes_gerais'],
                                'plano_de_acao': plano_de_acao_gerado,
