...
def get_consolidated_status_df(df_full):
    ...
    # --- CORREÇÃO 1: Usar 'numero_identificacao' como a chave única ---
    unique_ids = df_full['numero_identificacao'].unique()

    for ext_id in unique_ids: # Agora ext_id é o ID do equipamento
        # --- CORREÇÃO 2: Filtrar pelo ID do equipamento ---
        ...
        # --- CORREÇÃO 3: Montar o dicionário com a chave correta ---
        consolidated_data.append({
            'numero_identificacao': ext_id,
            'numero_selo_inmetro': latest_record.get('numero_selo_inmetro'), # Pega o último selo associado
...
def show_dashboard_page():
    ...
    with tab_extinguishers:
        ...
        # --- CORREÇÃO 4: Renomear as colunas para a exibição correta ---
        display_df = filtered_df.rename(columns={
            'numero_identificacao': 'ID do Equipamento', 
            'numero_selo_inmetro': 'Último Selo INMETRO', 
            'tipo_agente': 'Tipo',
            'status_atual': 'Status', 
        ...
        st.dataframe(
            styler,
            column_order=[col for col in display_df.columns if col != 'cor'],
            hide_index=True,
