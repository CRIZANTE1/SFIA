...
import pandas as pd
import numpy as np # Importa o numpy diretamente
import sys
import os
...
def assign_visual_properties(df):
    ...
    color_map = {
        'ABC': [255, 255, 0, 180], 'BC': [0, 0, 255, 180],
        'CO2': [128, 128, 128, 180], 'ÁGUA': [0, 100, 255, 180],
        'ESPUMA': [0, 128, 0, 180],
    }
    ...
    # Mapeia a capacidade para um tamanho no mapa
    # Adicionamos um valor base para que extintores pequenos ainda sejam visíveis
    df['size'] = 20 + (df['capacidade_num'] * 5)
    ...
    if equip_type == "Extintores":
        ...
        with st.expander("Ver detalhes e legenda"):
            ...
            st.markdown("""
                ...
                - <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background-color:rgba(0,0,255,0.8);"></span> Pó Químico BC
                ...
                - <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background-color:rgba(0,100,255,0.8);"></span> Água
                - <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background-color:rgba(0,128,0,0.8);"></span> Espuma
